...
class ChatGPTManager:
    ...
    def __init__(self, model: str = "gpt-3.5-turbo", first_system_prompt: str | None = None):
        ...
        if first_system_prompt is not None:
            self.FIRST_SYSTEM_MESSAGE = {"role": "system", "content": first_system_prompt}
    ...
    def chat_with_history(self, prompt: str | None = None) -> str | None:
        ...
        while num_tokens_from_messages(self.current_chat_history, self.model) > self.TOKEN_LIMIT:
            self.current_chat_history.pop(1) # We skip the 1st message since it's the system message
            print(f"[red]Popped a message! New token length is: {num_tokens_from_messages(self.current_chat_history, self.model)}")
        ...
        self.current_chat_history.append({"role": completion.choices[0].message.role, "content": completion.choices[0].message.content})
...
if __name__ == "__main__":
    ...
    chatgpt_manager = ChatGPTManager(model="gpt-3.5-turbo", first_system_prompt=first_system_promt)
    chatgpt_answer = chatgpt_manager.chat_with_history("Hey ScottBot, can you tell me what the cpu is?")
    quit()